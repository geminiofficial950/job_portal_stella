...
const BrandLogos: Record<string, React.ReactNode> = {
  ...
  BMW: (
    <div className="w-10 h-10 rounded-2xl bg-white border-2 border-slate-200 flex items-center justify-center">
      <span className="font-black text-[9px] tracking-tighter text-slate-900">
        BMW
      </span>
    </div>
  ),
  IBM: (
    <div className="w-10 h-10 rounded-2xl bg-[#EFF6FF] flex items-center justify-center">
      <span className="font-black text-[#006699] text-xs tracking-wider">
        IBM
      </span>
    </div>
  ),
  ...
  return (
    <section className="py-16 sm:py-20 px-4 sm:px-6 lg:px-8 bg-white border-b border-slate-100">
      <div className="max-w-5xl mx-auto">
        <div
          ref={headingRef}
          className="reveal flex flex-col md:flex-row md:items-end justify-between mb-8 gap-4"
        >
          <div>
          ...
          <button
            type="button"
            className="inline-flex items-center gap-1.5 text-xs font-bold text-[#b91c1c] hover:underline shrink-0 self-start md:self-auto"
          >
          ...
          {filterTabs.map((tab) => (
            <button
              ...
              style={{
                ...
                boxShadow:
                  activeTab === tab ? "0 4px 12px rgba(15,23,42,0.18)" : "none",
        ...
        <div
          ref={gridRef}
          className="reveal-stagger grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4"
        >
            ...
            return (
              ...
              >
                <div className="bg-slate-50 rounded-2xl p-4">
                  <div className="flex items-start justify-between mb-3">
                    ...
                    <button
                      ...
                      style={{
                        background: isBookmarked ? "#dc2626" : "#fff",
                  ...
                  <p className="text-xs font-semibold text-slate-400 font-inter mb-0.5">
                    {job.company}
                  </p>
                  <h3 className="text-base font-bold text-slate-900 font-manrope mb-3">
                    {job.title}
                  </h3>
                  <div className="flex flex-wrap gap-1.5">
                    {[job.workModel, job.type].map((tag) => (
                      <span
                        key={tag}
                        className="px-3 py-1 rounded-xl text-[11px] font-semibold bg-white border border-slate-200 text-slate-600 font-inter"
                      >
                        {tag}
                      </span>
                    ))}
                ...
                <div className="flex items-center justify-between px-1">
                  <div>
                    <p className="font-extrabold text-slate-900 text-sm font-manrope">
                      {job.salary}
                    </p>
